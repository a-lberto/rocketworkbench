...
#include <iostream>
#include <cstdlib>
#include <simulator/lsode.h>
//#include <simulator/simulation.h>
#include <simulator/c++rocket.h>
...
int lsode::solve(double *st, double duration, double step) 
{
  ...
  for (int i = 0; i < length; i++) {
    ...
    lsode_(md, neq, st, time, tout, itol, rtol, atol, 
...
char *error_msg(State s)
{
  ...
  {
  case TOO_MUCH_WORK:
    return (char*)"Excess work done on this call (perhaps wrong mf).";
    break;
    
  case TOO_MUCH_ACC:
    return (char*)"Excess Accuracy Requested (Tolerances too small).";
    break;
    
  case ILLEGAL_INPUT:
    return (char*)"Illegal input detected.";
    break;
    
  case ERR_FAILURE:
    return (char*)"Repeated error test failures.";
    break;
    
  case CONV_FAILURE:
    return (char*)"Repeated convergence failures.";
    break;
    
  case ERROR_WEIGHT:
    return (char*)"Error weight became zero during problem.";
    break;
    
  case EXIT_IN_FUNCTION:
    return (char*)"Exit requested in ode function.";
    break;
    
  default:
    return (char*)"Unrecognize error state.";
